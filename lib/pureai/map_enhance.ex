defmodule PureAI.MapEnhance do
  @moduledoc """
  ...
  def atomize_keys(map = %{}) do
    map
    |> Enum.map(fn {k, v} -> do_atom_key({k, v}) end)
    |> Enum.into(%{})
  end
  ...
  defp do_atom_key({key, value}) when is_binary(key),
    do: {String.to_atom(key), atomize_keys(value)}

  defp do_atom_key({key, value}) when is_atom(key), do: {key, atomize_keys(value)}
